/**
 * Returns 0 if BAD or NO is recieved
 * Returns 1 if OK recieved
 */
bool check_response(std::string &response, std::string &id_string){
    ...
    if (std::regex_match (response, std::regex(BADrgx) )){
        std::cout << "[ERROR] BAD command: " << response;
        return 0;
    }
...
bool imap::createMailbox(std::string &mailbox){
    std::string id_string = "a";
    std::string command = id_string + " create " + mailbox +"\r\n";
    socket.send(command);
    std::string response = socket.recieve();
    return check_response(response, id_string);   
}

bool imap::deleteMailbox(std::string &mailbox){
    std::string id_string = "a";
...
bool imap::renameMailbox(std::string &oldmailbox, std::string &newmailbox){
    std::string id_string = "a";
...
int imap::getCount(const std::string &mailbox){
    std::string id_string = "a";
...
bool imap::deleteMail(Mail mail){
    std::string id_string = "a";
    ...
    std::string command = id_string + " uid store " + mail.uid + " +FLAGS (\Deleted)\r\n";
    socket.send(command);
    std::string response = socket.recieve();
    bool response_id = check_response(response, id_string);
    if (!response_id) return response_id;
    std::string command = id_string + " expunge " + mail.uid + "\r\n";
    socket.send(command);
    std::string response = socket.recieve();
    return response_id = check_response(response, id_string);
}

Mail imap::getMail(const std::string &mailbox, const std::string uid){
    Mail mail;
    ...
    std::string command = id_string + " uid fetch " + mail.uid + " (BODY[HEADER.FIELDS (from to subject date)])\r\n";
    socket.send(command);
    std::string response = socket.recieve();
    bool response_id = check_response(response, id_string);
    if (response_id){
    ...
    std::string command = id_string + " uid fetch " + mail.uid + " rfc822.text\r\n";
    socket.send(command);
    std::string response = socket.recieve();
    bool response_id = check_response(response, id_string);
    if (response_id){
